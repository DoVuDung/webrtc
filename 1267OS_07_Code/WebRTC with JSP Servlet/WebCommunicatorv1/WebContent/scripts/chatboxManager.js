...
var chatboxManager = function() {
	...
	if(idx1 != -1) {
	    ...
	    boxList.push(id);
	    showList.push(id);
	    nameList.push((user.first_name).toLowerCase());
	}
    };

    var messageSentCallback = function(id, user, msg) {
    //------------------------------------------------------
    //Control comes from "self.options.messageSent(self.options.id, self.options.user, msg);"
    //to chatBoxManager for sending the message to users
