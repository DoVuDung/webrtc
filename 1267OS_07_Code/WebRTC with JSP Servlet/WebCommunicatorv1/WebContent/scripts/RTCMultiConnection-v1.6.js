...
(function() {
	...
	//alert("1222");
	//var pawan=prompt("enter your chatname is","");
	//alert("4545");
	var username2="";
	var username="";
    window.RTCMultiConnection = function(str) {
        // www.RTCMultiConnection.org/docs/channel-id/
    	username2=str;
    	this.channel2=location.href;
        this.channel =location.href.replace( /\/|:|#|%|\.|\[|\]/g , '');
        //alert("channel name is--"+this.channel2);
            ...
            function _captureUserMedia(forcedConstraints, forcedCallback, isRemoveVideoTracks) {
                var mediaConfig = {
                    ...
                    onerror: function(e) {
                        ...
                        //alert("toStr(e)"+toStr(e));
                         
                        if (session.audio) {
                        	//alert("Maybe microphone access is denied or not available");
                            self.onMediaError('Maybe microphone access is denied.');
                        }

                        if (session.video) {
                        	//alert("Maybe webcam access is denied or disconnected or not available");
        ...
        function newPrivateSocket(_config) {
            ...
            var peerConfig = {
                ...
                onicecandidate: function(candidate) {
                	//alert("on ice candidate---"+candidate.sdpMLineIndex);
                	//alert("on ice candidate--"+JSON.stringify(candidate.candidate));
                ...
                oniceconnectionstatechange: function(event) {
                	//alert("in oniceconnectionstatechange");
                	//var onice=toStr(event);
    ...
    function PeerConnection() {
        return {
            ...
            addIceCandidate: function(candidate) {
            	//alert("on add  addIceCandidate"+candidate.sdpMLineIndex);
            	//alert("on add  addIceCandidate"+candidate.candidate);
    ...
    var FileSender = {
        send: function(config) {
           ...
           var extra=config.extra;
            ...
            var packets = 0;
            if(connection.extra.type==="private")
            	{
    ...
    function setDefaults(connection) {
        ...
        connection.onmessage = function(e) {
        	//alert("connect message is--"+toStr(e));
        ...
        connection.onerror = function(e) {
        	//alert("connection is not established between you and user"+toStr(e));
        ...
        connection.token = function() {
            ...
            if (window.crypto) {
               ...
               // return token;
				//var token12=prompt("enter your name chatroom name","")||UNKNOWN;
                return token;
            } else {
                return (Math.random() * new Date().getTime()).toString(36).replace( /\./g , '');
				//var token123=prompt("enter your name chatroom name","")||UNKNOWN;
                return token;
            }
        };
		...
		connection.userid =username2;
	    username=connection.userid;
        connection.getusername=function(){
        ...
        //for(i=0;i<iceServers.length;i++)
        	//{
        	//alert("iceserver value is --"+iceServers[i].url);
        	//}
        ...
        connection.caniuse = {
            ...
            checkIfScreenSharingFlagEnabled: function(callback) {
                ...
                if (isFirefox) {
                    ...
                    alert("Screen sharing is NOT supported on Firefox.");
                    error(warning);
                ...
                if (chromeVersion < 26) {
                    ...
                    alert("Screen sharing support is suspicious! Chrome Version is less than 26");
                    warn(warning);
                }
