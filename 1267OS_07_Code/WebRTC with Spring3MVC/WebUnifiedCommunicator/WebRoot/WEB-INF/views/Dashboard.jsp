        ...
        btnRegister.disabled = true;
            ...
            oSipStack = new SIPml.Stack({
                    ...
                    sip_headers: [
                            ...
                            { name: 'Organization', value: 'Independant' }
                    ]
                }
            );
...
<script>
        function viewFrnd(){
            user = "${userdetail.privateIdentity}";
                $.ajax({  
                    type: "POST",  
                    url: "/WebUnifiedCommunicator/addgeolocation.html",  
                    data: "sipuri="+user,  
                    success: function(response){                                   
         ...
         alert("ry======${geolocations}");      
       
            var all =[
                     <c:forEach items="${geolocations}" var="geolocation">
                     ["${(geolocation.sipuri)}","${(geolocation.latitude)}","${(geolocation.longitude)}"],
                    </c:forEach>         
                    ];        
              alert(all[0]+"  Lat : "+all[1]+ "  Long :  "+ all[2]);
         ...
         // Set the Map variable
                ...
                map = new google.maps.Map(document.getElementById('map_canvas'),
                  ...
                  alert(" maps initialise latitude="+latitude+" longitude"+longitude);
                  ...
                  var pos1 = new google.maps.LatLng(all[i][1],all[i][2]);
            ...
            alert(" maps initialised and loaded ");
    </script>
...
<td>
       <img id="userImage1" src="<c:url value="UserImage/${userdetail.displayName}.png"/>">		<!-- Profile Modal starts  -->
		...
		<h5>Your Profile</h5>
		<form:form method="POST" action="/WebUnifiedCommunicator/saveuserdetail.html">
		  <table>
		   <tr>
...
<td>
		...
		document.getElementById("notFrndDiv").innerHTML="";
		  ...
		  jsonObject=eval('('+response+')');
		   	...
		   	if(k.match("Friendsipuri")){
		   		 $('<td><img src="<c:url value="UserImage/'+friendimg+'.png"/>" style="width: 30px;height: 30px;"></td>').appendTo(tr); 
		...
		<div style="margin-top: 5%;max-height:130px;overflow:auto;" id="notFrndDiv">	</div> 
		...
		<br/></br>
		<!-- geolocation model  -->
		 <div id="geoLocationModal" style=" background-color:#a9a9a9; " >
		        <h5>Geolocation Box</h5>
		        <script>
		            function doAjaxPostGeolocations() {
		                            ...
		                            var date =arr[1];
		                            var time = arr[2];
		                            alert (" contact data "+ sipuri+" lat"+ latitude+" long"+longitude+" "+date+" "+time);
		                              
		                            $.ajax({  
		                                type: "POST",  
		                                url: "/WebUnifiedCommunicator/addgeolocationajax.html",  
		                ...
		                <table>
		                <tr>
		                <td align="center" style="display:block;padding-left:5px;"><input type="button" class="button" id="shareGeoLoc" onclick="doAjaxPostGeolocations()" value="Share my location"/>
		                </tr>
		                </table>
		    
		            <div id="infogeolocation" style="color: green;"></div> 
		            <div id="map_canvas" style="width:100%;height:80%;"> Map</div>     
